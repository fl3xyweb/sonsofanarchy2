...
const debtInterestMode = document.getElementById("debtInterestMode");
...
const debtLateRate = document.getElementById("debtLateRate");
...
const calculateDebtInterest = (principal, rate, startDate, dueDate, lateRate = 0) => {
  const start = startDate ? new Date(startDate) : null;
  const end = dueDate ? new Date(dueDate) : null;
  if (!start || !end || Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return 0;
  const msPerDay = 1000 * 60 * 60 * 24;
  const days = Math.max(0, Math.ceil((end - start) / msPerDay));
  if (!principal || days <= 0) return 0;
  const baseInterest = rate ? (principal * (rate / 100) * (days / 365)) : 0;
  const now = new Date();
  const lateDays = Math.max(0, Math.ceil((now - end) / msPerDay));
  const lateInterest = lateRate && lateDays > 0
    ? (principal * (lateRate / 100) * lateDays)
    : 0;
  return Math.round(baseInterest + lateInterest);
};
...
const updateDebtTotals = (items) => {
  ...
  items.forEach((item) => {
    ...
    const interest = item.interestMode === "auto"
      ? calculateDebtInterest(
          principal,
          Number(item.interestRate || 0),
          item.startDate,
          item.dueDate,
          Number(item.lateRate || 0)
        )
...
function renderDebts(readOnly = false) {
  ...
  debtTableBody.innerHTML = items
    .map((item, index) => {
      const principal = Number(item.principal || 0);
      const computedInterest = item.interestMode === "auto"
        ? calculateDebtInterest(
            principal,
            Number(item.interestRate || 0),
            item.startDate,
      ...
      const interest = item.interestMode === "auto"
        ...
        : Number(item.interestAmount || 0);
      const total = principal + interest;
      const lateRate = Number(item.lateRate || 0);
      const interestLabel = item.interestRate
        ? `${item.interestRate}% · ${formatCurrency(interest)}${lateRate ? ` (+${lateRate}%/den)` : ""}`
...
const updateDebtTotalPreview = () => {
  ...
  const mode = debtInterestMode?.value || "auto";
  if (mode === "auto" && rate > 0) {
...
const openDebtModal = (item = null, index = null) => {
  ...
  if (!item) {
    ...
    if (debtInterestMode) debtInterestMode.value = "auto";
    ...
    if (debtLateRate) debtLateRate.value = "";
    ...
    if (debtInterestMode) debtInterestMode.value = item.interestMode || "auto";
    ...
    if (debtLateRate) debtLateRate.value = String(item.lateRate || "");
...
debtInterestMode?.addEventListener("change", updateDebtTotalPreview);
debtStartDate?.addEventListener("change", updateDebtTotalPreview);
debtDueDate?.addEventListener("change", updateDebtTotalPreview);
debtLateRate?.addEventListener("input", updateDebtTotalPreview);
...
debtForm?.addEventListener("submit", (event) => {
  ...
  const rate = Number(debtInterestRate?.value || 0);
  const mode = debtInterestMode?.value || "auto";
  const interestAmount = mode === "auto"
    ? calculateDebtInterest(
        principal,
        rate,
        debtStartDate?.value,
    ...
    : Number(debtInterestAmount?.value || 0);
  const payload = {
    ...
    interestAmount,
    interestMode: mode,
    lateRate: Number(debtLateRate?.value || 0),
